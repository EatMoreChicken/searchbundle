...
function CustomTooltip({
  ...
  return (
    <div className="rounded-xl border border-border bg-elevated px-4 py-3 shadow-lg text-[13px]">
      ...
      {expected != null && (
        <p className="font-semibold text-text">Expected: <span className="text-teal">{fmt(expected)}</span></p>
      )}
      ...
      {includeInflation && inflAdj != null && (
        <p className="text-text-tertiary mt-1">Inflation-adj: {fmt(inflAdj)}</p>
      )}
    </div>
  );
}
    ...
    <ResponsiveContainer width="100%" height={320}>
      <ComposedChart data={data} margin={{ top: 8, right: 16, bottom: 0, left: 0 }}>
        ...
        <Line
          ...
          dot={false}
          name="Expected"
          activeDot={{ r: 5, fill: "var(--color-teal)", stroke: "var(--color-elevated)", strokeWidth: 2 }}
        />
